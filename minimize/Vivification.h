...
public:
	...
	Vivification(SolverConfig const & config, Database & ca,
			ImplicationGraph<Database> & ig, Propagate & propEngine);
...
template<typename Propagate>
inline Vivification<Propagate>::Vivification(const SolverConfig& config,
		Database & ca, ImplicationGraph<Database>& ig, Propagate& propEngine) :
		ca(ca), ig(ig), propEngine(propEngine) {
}
...
inline typename Vivification<Propagate>::PropagateResult Vivification<Propagate>::propagateClause(
	...
	for (int i = 0; i < simp_learnt_clause.size(); ++i) {
		if (ig.value(simp_learnt_clause[i]).isUndef()) {
			++res.numProps;
			propEngine.simpleUncheckEnqueue(~simp_learnt_clause[i],Database::npos());
			if ((res.confl = propEngine.simplePropagate()) != Database::npos())
				break;
		} else {
			if (ig.value(simp_learnt_clause[i]).isTrue()) {
				res.isTruePropagation = true;
				res.confl = ig.reason(simp_learnt_clause[i]);
...
template<typename Propagate>
inline int Vivification<Propagate>::simpleAnalyze(CRef confl, vec<Lit>& c,
		const bool True_confl) {
	int pathC = 0;
	Lit p = Lit::Undef();
	...
	do {
		if (confl != Database::npos()) {
			Clause& c = ig.getResolvent(confl);
			if (p != Lit::Undef() && c.size() == 2
					&& ig.value(c[0]).isFalse()) {
				assert(ig.value(c[1]).isTrue());
				Lit tmp = c[0];
				c[0] = c[1], c[1] = tmp;
			}
			// if True_confl==true, then choose p begin with the 1th index of c;
			for (int j = (p == Lit::Undef() && True_confl == false) ? 0 : 1;
					j < c.size(); j++) {
				Var const v = c[j].var();
				if (!ig.isSeen(v)) {
					ig.setSeen(v);
